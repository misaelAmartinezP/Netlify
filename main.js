...
function parceoDatos(string) {
    ...
    generaIds = "declare @idpro int\n    declare @idctas int \n declare @numcta varchar(15) \n set @idpro = (select MAX(AlyConstructora.dbo.AcProveedores.idProveedor) from AlyConstructora.dbo.AcProveedores); \n    set @idctas = (select MAX(AlyConstructora.dbo.CtbCuentas.idCuenta) from AlyConstructora.dbo.CtbCuentas); \n set @numcta = ((select cuenta from AlyConstructora.dbo.CtbCuentas where idCuenta =@idctas) -11801010000 + 1); \n select @idpro \n select @idctas \n  select @numcta"
